...
### A.5  LOOP Canonical Chain Enforcement (No Wrapped Representation)

LOOP exists exclusively on BNB Chain (BSC).

There is no wrapped LOOP (wLOOP), no bridge-backed representation, and no secondary supply on any spoke chain.

This is a deliberate invariant-enforced design decision.

LOOP is a non-transferable, epoch-isolated redemption certificate representing realized, closed profit generated by the BSC Hub Vault. Because LOOP is strictly tied to BSC-based accounting invariants (coverage, ratchet, epoch isolation), it must remain on a single canonical chain.

Replicating LOOP across chains would introduce:

- Cross-chain accounting desynchronization risk
- Independent liability states
- Bridge escrow management complexity
- Expanded smart contract attack surface
- Multi-chain floor enforcement challenges
...
• LOOP minting occurs on BSC only.  
• LOOP redemption occurs on BSC only.  
• Redemption_price_E is computed and updated on BSC only.  
• Coverage_ratio_E is computed and enforced on BSC only.  
• LOOP is non-transferable.  
• LOOP cannot exist on any chain other than BSC.  

These rules are enforced at the contract level within HubVault.sol.

---

#### Multi-Chain Reward Delivery (Revised)

At cycle close, reward delivery logic operates as follows:

If the user selects **USDT**:
- USDT is released from the BSC Hub Vault.
- If the destination chain is BSC → direct transfer.
- If the destination chain is a spoke chain → USDT is bridged via Stargate.
- Bridge fees are deducted from the Friction Pool as operational cost.

If the user selects **LOOP**:
- LOOP is minted on BSC only.
- LOOP remains on BSC.
- No cross-chain bridge action occurs.

If the user later redeems LOOP:
1. Redemption is executed on BSC.
2. USDT is released from Redemption_pool_E.
3. If desired, USDT may be bridged to another supported chain.

There is no mechanism to represent LOOP on spoke chains.

---

#### Contract Implications

The following components are removed from the architecture:

• wLOOPBridge.sol  
• Bridge escrow contracts for LOOP  
• wLOOP supply tracking logic  
• wLOOP mint/burn mechanics  

SpokeGateway.sol continues to support:
• Deposit ingress (USDT → BSC)
• Withdrawal egress (USDT ← BSC)

SpokeGateway does NOT manage LOOP balances or accounting.

All LOOP state remains exclusively within HubVault.sol on BSC.

---

#### Security Impact

Removing wrapped LOOP:

• Eliminates escrow liability risk  
• Eliminates bridge-backed token supply synchronization risk  
• Reduces audit scope  
• Reduces multisig authority surface  
• Strengthens invariant enforcement  
• Simplifies cross-chain threat model  

YieldLoop enforces:

Single-chain accounting.  
Single-chain liability.  
Multi-chain access.  
No cross-chain token replication.
---
